...
def index():
    return reactstub("Heffalump", ["app/css/styles.css", "app/css/slider.css"], ["app/main.js"], bootstrap=json.dumps({"sessionID": None}))


@app.route("/<session_id>", methods=['GET'], strict_slashes=False)
def role_assignment(session_id):
    return reactstub("Heffalump", ["app/css/styles.css", "app/css/slider.css"], ["app/main.js"], bootstrap=json.dumps({"sessionID": session_id, "roleID": None}))


@app.route("/<session_id>/<role_id>", methods=['GET'], strict_slashes=False)
def view(session_id, role_id):
    return reactstub("Heffalump", ["app/css/styles.css", "app/css/slider.css"], ["app/main.js"], bootstrap=json.dumps({"sessionID": session_id, "roleID": role_id}))
